...
// Dominios permitidos a llamar este worker (ajustá si tu sitio tiene otro dominio)
const ALLOWED_ORIGINS = ['*']; // '*' = cualquier origen. Podés restringirlo a tu dominio exacto.
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/db' && request.method === 'POST') {
      try {
        ...
        // Obtenemos el sha actual (necesario para no pisar cambios de otro)
        ...
        // Si hubo conflicto (alguien escribió justo antes), reintentamos una vez con el sha fresco
